...
class CentralEnt:
	...
	def addNode2Graph(self, graphMaker):
		self.nodeId = graphMaker.addNode(self.content)
...
class AuthorEnt(CentralEnt):
	...
	def extendRels(self, graphMaker):
		...
		for rel in relset:
			...
			dstNodeId = graphMaker.addNode(poem.poem_name)
...
class PoemEnt(CentralEnt):
	...
	def extendRels(self, graphMaker):
		...
		for rel in relset1:
			...
			node.addNode2Graph(graphMaker)
			dstNodeId = graphMaker.addNode(author.author_name)
			newNodes.append(node)
			graphMaker.addLink(self.nodeId, dstNodeId, u"撰写")
		...
		for rel in relset2:
			...
			node.addNode2Graph(graphMaker)
			dstNodeId = graphMaker.addNode(image.image_name)
			newNodes.append(node)
...
class ImageEnt(CentralEnt):
	...
	def extendRels(self, graphMaker):
		...
		for rel in relset2:
			...
			node.addNode2Graph(graphMaker)
			dstNodeId = graphMaker.addNode(emotion.emotion_desc)
			newNodes.append(node)
...
class EmotionEnt(CentralEnt):
	...
	def executeQuery(self):
		qset = Emotion.objects.filter(emotion_desc__contains=self.content)
		if not qset.exists():
			return False
		self.record = qset[0]
	...
	def extendRels(self, graphMaker):
		newNodes = []
		relset = Image_Emotion.objects.filter(emotion=self.record.emotion_id)
		for rel in relset:
			image = rel.image
			node = ImageEnt(image.image_name, image)
			node.addNode2Graph(graphMaker)
